...
if [ -f "$1" ]; then
    while IFS='' read -r line || [[ -n "$line" ]]; do
        ...
        # shared .env means the same thing on both platforms — a stray space
        # around '=' can't become a space-prefixed credential, and a # comment
        # can't break the generated Env.swift.
        case "$trimline" in
            ''|'#'*) continue ;;
        esac
